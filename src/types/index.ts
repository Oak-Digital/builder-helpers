...
type EnumableTypes = 'text' | 'string' | 'email' | 'number';
...
type RequiredFallback<T extends BaseInputObject> = T['required'] extends true
    ? never
    : undefined;
/**
...
export type InputObject =
    | PrimitiveInputObject
    | UnknownInputObject
    | EnumInputObject
    | ListInputObject;

type InferEnumType<T extends EnumInputObject> = T['enum'][number];
type InferListType<T extends ListInputObject> = InputObjectArrayToProps<
    T['subFields']
>[];
type InferObjectType<T extends ObjectInputObject> = InputObjectArrayToProps<
    T['subFields']
>;
type InferInputBaseType<T extends InputObject> = T extends EnumInputObject
    ? InferEnumType<T>
    : T extends ListInputObject
    ? InferListType<T>
    : T extends ObjectInputObject
    ? InferObjectType<T>
    : T extends PrimitiveInputObject
    ? BuilderInputPrimitiveTypeMap[T['type']]
    : unknown;
type InferInputType<T extends InputObject> =
    | InferInputBaseType<T>
    | InputObjectFallback<T>;
...
type InputObjectArrayToProps<T extends readonly InputObject[]> = Merge<
    InputObjectArrayToPropsArray<T>[number]
> & {
    attributes: {
        'builder-id': string;
        [key: string]: any;
...
export type RegisterObjectToProps<
    T extends { inputs: readonly InputObject[] }
> = InputObjectArrayToProps<T['inputs']>;
